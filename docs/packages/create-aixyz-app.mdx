...
- **Ollama base URL** — Saved to `.env.local` (defaults to `http://127.0.0.1:11434/api`)
...
Use `--ollama-base-url` to set a custom Ollama endpoint:

```bash
bunx create-aixyz-app my-agent --ollama-base-url http://my-ollama-host:11434/api
```
