...
Loads environment variables from `.env` files for test runs. Follows the same loading order as Next.js, with one exception: `.env.local` is **not loaded** during tests (use `.env.test.local` instead).
