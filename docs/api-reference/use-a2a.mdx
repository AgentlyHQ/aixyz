...
- If `exports.accepts` has `scheme: "exact"`, the `/agent` endpoint is gated behind x402 payment
- If `exports.accepts` has `scheme: "free"`, the `/agent` endpoint is registered without payment gating
