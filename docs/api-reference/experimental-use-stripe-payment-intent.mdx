---
title: "StripePaymentIntentPlugin"
...
After registering the `StripePaymentIntentPlugin`, the server checks incoming requests for the `x-stripe-payment-intent-id` header:
...
<Note>
  Register the `StripePaymentIntentPlugin` **before** A2A or MCP plugins so the Stripe middleware runs first. If a valid
  Stripe payment is found, x402 verification is skipped.
</Note>
